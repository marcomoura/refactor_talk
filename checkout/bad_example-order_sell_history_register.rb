module EvMain
  class OrderSellHistoryRegister
    def initialize(order, logger = LogManager)
      @order = order
    ...
    def perform_pre_vendas!
      @order.available_packages.each do |package|
        begin
          gateway = format_payment_type(@order.gateway_type, package)
          venda_record = VendasRecord.where(order_id: package.external_id).first
          venda_record.update_attributes(fpagamento: gateway,
          ...
          create_vendas_taxas(venda_record, package)

          create_balcao(package)
          create_vendas_parcelamento if should_create_vendas_parcelamento?
        end
      end
    end
